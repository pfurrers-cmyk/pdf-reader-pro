<script lang="ts">
  // ========================================================================
  // COMMAND PALETTE — Paleta de comandos estilo VS Code (Ctrl+K)
  // ========================================================================
...
{#if $commandPaletteOpen}
  ...
  >
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div
      class="w-full max-w-lg bg-zinc-800 rounded-xl shadow-2xl border border-zinc-700 overflow-hidden"
      onclick={(e) => e.stopPropagation()}
    >
      <!-- Search input -->
      <div class="flex items-center px-4 h-12 border-b border-zinc-700">
        <svg class="w-4 h-4 text-zinc-500 mr-3 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        {#each filteredCommands as cmd, i (cmd.id)}
          <button
            class="w-full flex items-center justify-between px-4 py-2.5 text-sm transition-colors cursor-pointer
                   {i === selectedIndex ? 'bg-blue-600/20 text-white' : 'text-zinc-300 hover:bg-zinc-750'}"
            onclick={() => executeCommand(cmd)}
            onmouseenter={() => (selectedIndex = i)}
          >
            <div class="flex items-center gap-2">
              <span class="text-[10px] text-zinc-600 uppercase tracking-wider w-16">{cmd.category}</span>
              <span>{cmd.label}</span>
            </div>
            {#if cmd.shortcut}
              <kbd class="text-[10px] text-zinc-600 bg-zinc-900 px-1.5 py-0.5 rounded">{cmd.shortcut}</kbd>
            {/if}
          </button>
        {/each}

        {#if filteredCommands.length === 0}
          <div class="px-4 py-8 text-center text-sm text-zinc-600">
