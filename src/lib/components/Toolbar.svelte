<script lang="ts">
  // ========================================================================
  // TOOLBAR — Barra de ferramentas principal do PDF Reader Pro
  // ========================================================================
  import { activeTab, viewMode, ZOOM_PRESETS, DEFAULT_ZOOM } from '$lib/stores';
  ...
  function zoomIn() {
    ...
    if (idx < ZOOM_PRESETS.length - 1) {
      onZoomChange(ZOOM_PRESETS[idx + 1]);
    }
  }
...
<div class="flex items-center gap-0.5 px-2 h-9 bg-zinc-850 border-b border-zinc-700 text-zinc-300 select-none">
  ...
  <span class="text-xs text-zinc-400 min-w-[60px] text-center">
    {#if $activeTab}
      {$activeTab.currentPage + 1} / {$activeTab.pageCount}
    {:else}
      – / –
    {/if}
  </span>
  ...
  <button
    class="text-xs text-zinc-300 hover:text-white px-2 py-1 rounded hover:bg-zinc-700 transition-colors min-w-[50px] text-center cursor-pointer"
    onclick={zoomReset}
  ...
  <ToolbarButton title="Ajustar à largura" onclick={onFitWidth}>
    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M21 3H3v18h18V3z"/><path d="M3 9h18M3 15h18"/>
    </svg>
  </ToolbarButton>
  ...
  <ToolbarButton title="Rolagem contínua" onclick={() => setViewMode('continuous')} active={$viewMode === 'continuous'}>
    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <rect x="6" y="2" width="12" height="6" rx="1"/><rect x="6" y="9" width="12" height="6" rx="1"/><rect x="6" y="16" width="12" height="6" rx="1"/>
    </svg>
  </ToolbarButton>
  ...
  <!-- Search -->
  <ToolbarButton title="Buscar (Ctrl+F)" onclick={() => {}}>
    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
    </svg>
  </ToolbarButton>
  ...
  <!-- Command palette -->
  <ToolbarButton title="Paleta de comandos (Ctrl+K)" onclick={() => {}}>
    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
